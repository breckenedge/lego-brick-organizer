...
export class PartCard {
  static render(part, showAssignButton = false) {
    ...
    const assignButton = showAssignButton
      ? `<button class="btn-secondary" data-action="assign-to-bin" data-part-num="${part.part_num}" data-part-name="${part.name}">Assign to Bin</button>`
      : '';

    return `
      <div class="part-card">
        <div class="part-image">
          <img src="${PartsAPI.getPartDrawingURL(part.part_num)}"
               alt="${part.name}"
               onerror="this.src='data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22200%22 height=%22200%22><rect fill=%22%23f0f0f0%22 width=%22200%22 height=%22200%22/><text x=%2250%25%22 y=%2250%25%22 text-anchor=%22middle%22 dy=%22.3em%22 fill=%22%23999%22 font-family=%22Arial%22 font-size=%2214%22>No image</text></svg>'">
        </div>
        <div class="part-info">
          <h3>${part.part_num}</h3>
